...
trait mostrarUsuarios{

    static function mostrarUsuariosAdmin($usuarios){
...
class UsuariosController extends Usuario{
    ...
    public function mostrarTodosLosUsuarios(){
        try{
            ...
            self::mostrarUsuariosAdmin($usuarios);

        }catch(Exception $e){
            exit("ERROR AL MOSTRAR LOS USUARIOS: ".$e->getMessage());
        }
    }

    public function buscarUsuarios($nombre, $apellidos, $correo, $telefono, $cedula){

        try{

            $usuarios = parent::searchUsuarios($nombre, $apellidos, $correo, $telefono, $cedula);
